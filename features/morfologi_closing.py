...
def apply_closing(main_window, kernel_type="square", size=3):
    ...
    main_window.show_progress(f"Applying Closing ({kernel_type} {size})...")
    ...
    # Apply closing
    closed = binary_closing(binary, structure=selem).astype(np.uint8) * 255

    output_img = Image.fromarray(closed)
