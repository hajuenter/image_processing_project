...
def apply_opening(main_window, kernel_type="square", size=3):
    ...
    main_window.show_progress(f"Applying Opening ({kernel_type} {size})...")
    ...
    # Apply opening
    opened = binary_opening(binary, structure=selem).astype(np.uint8) * 255

    output_img = Image.fromarray(opened)
