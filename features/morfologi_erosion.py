...
def apply_erosion(main_window, kernel_type="square", size=3):
    ...
    main_window.show_progress(f"Applying Erosion ({kernel_type} {size})...")
    ...
    # Apply erosion
    eroded = binary_erosion(binary, structure=selem).astype(np.uint8) * 255

    output_img = Image.fromarray(eroded)
